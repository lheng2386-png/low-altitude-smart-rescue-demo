...
def _quality_preset():
    return os.getenv("ARGUS_YOLO_PRESET", "balanced").strip().lower()
...
class YOLOInferencer:
    def __init__(
        ...
        self.imgsz = imgsz
        self.device = device
        self.conf = _env_float("ARGUS_YOLO_CONF", 0.15) if conf is None else conf
        self.iou = _env_float("ARGUS_YOLO_IOU", 0.50) if iou is None else iou
        self.augment = _env_flag(
            "ARGUS_YOLO_AUGMENT",
            _quality_preset() in {"max", "ultra", "strongest"},
    ...
    def run(self, images):
        ...
        for idx, image in enumerate(images):
            ...
            detections = self.infer_image(
                img,
                depths=_env_int("ARGUS_YOLO_TILE_DEPTHS", 3 if _quality_preset() in {"max", "ultra", "strongest"} else 2),
                overlap=_env_float("ARGUS_YOLO_TILE_OVERLAP", 0.30 if _quality_preset() in {"max", "ultra", "strongest"} else 0.25),
                batch_size=_env_int("ARGUS_YOLO_TILE_BATCH_SIZE", 4 if _quality_preset() in {"max", "ultra", "strongest"} else 8),
            )
