...
- 修改 `app/app.py`
  ...
  - Gradio 中使用文件上传组件读取 mask，避免自动图像模式转换破坏单通道 class-id mask。
...
1. 单通道 class id mask
   - 推荐文件格式：`.png`
   - 不建议使用 `.jpg` 保存 class-id mask，因为 JPG 压缩可能改变像素类别值。
   - 每个像素值为类别 id：0 到 10。
...
2. RGB 彩色 mask
   ...
   - Palette/P 模式 mask 会先转换为 RGB，再按 RescueNet 颜色映射转换为 class id。

如果 mask 尺寸和原图尺寸不同，系统会使用最近邻插值自动 resize 到图像尺寸，保证类别 id 不被插值破坏。

Segmentation overlay 只对非 `background` 区域叠加颜色，`background` 保持原图，避免整张图被背景类别压暗。
...
```text
app/examples/masks/demo_rescuenet_mask.png
```

可以在 Image Tab 中上传任意示例图片，再把该文件作为 `Optional Segmentation Mask Upload` 上传，用于验证 segmentation overlay、segmentation summary、environment_score 和带环境风险的报告输出。
