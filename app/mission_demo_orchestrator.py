...
def run_one_click_mission_demo(
    image,
    ...
    transformer_confidence_threshold=0.4,
    start_point=None,
    run_decision_fusion=True,
    generate_final_report=True,
    output_root=None,
):
    """Run the full mission demo flow while keeping every stage truthful."""
    normalized = normalize_demo_image_input(image)
    mission_id = datetime.now().strftime("mission_%Y%m%d_%H%M%S")
    base_root = Path(output_root) if output_root else DEFAULT_OUTPUT_BASE
    mission_root = _ensure_dir(base_root / mission_id)
    outputs_root = _ensure_dir(mission_root / "outputs")
    detection_dir = _ensure_dir(outputs_root / "detection")
    segmentation_dir = _ensure_dir(outputs_root / "segmentation_inference")
    decision_dir = _ensure_dir(outputs_root / "decision_fusion")
    thermal_dir = _ensure_dir(outputs_root / "thermal")
    reports_dir = _ensure_dir(outputs_root / "reports")

    stages = {}
    artifacts = []

    detection_stage = run_detection_stage(
    ...
    stages["detection"] = detection_stage
    artifacts.extend(detection_stage.get("artifacts", []))
    ...
    stages["segmentation"] = segmentation_stage
    artifacts.extend(segmentation_stage.get("artifacts", []))

    decision_stage = build_stage_result(
        "decision",
        DEMO_STAGE_STATUS["not_requested"],
        result=None,
        message="Decision stage not requested.",
        artifacts=[],
        truthfulness_note="Decision stage was not executed.",
    )
    if run_decision_fusion:
    ...
    stages["decision"] = decision_stage
    artifacts.extend(decision_stage.get("artifacts", []))

    thermal_stage = run_thermal_stage(
        thermal_image=thermal_image,
        thermal_mode=thermal_mode,
        output_dir=thermal_dir,
    )
    stages["thermal"] = thermal_stage
    artifacts.extend(thermal_stage.get("artifacts", []))

    report_stage = build_stage_result(
        "report",
        DEMO_STAGE_STATUS["not_requested"],
        result=None,
        message="Report stage not requested.",
        artifacts=[],
        truthfulness_note="Report stage was not executed.",
    )
    if generate_final_report:
        report_stage = run_report_stage(root_dir=mission_root, output_dir=reports_dir)
    stages["report"] = report_stage
    artifacts.extend(report_stage.get("artifacts", []))

    success_stage_count = sum(1 for stage in stages.values() if stage.get("status") == DEMO_STAGE_STATUS["success"])
    partial_stage_count = sum(1 for stage in stages.values() if stage.get("status") == DEMO_STAGE_STATUS["partial_success"])
    skipped_stage_count = sum(1 for stage in stages.values() if stage.get("status") == DEMO_STAGE_STATUS["skipped"])
    failed_stage_count = sum(1 for stage in stages.values() if stage.get("status") == DEMO_STAGE_STATUS["failed"])
    if failed_stage_count == len(stages):
        success = False
        partial_success = False
    else:
        success = failed_stage_count == 0 and success_stage_count >= 2
        partial_success = success_stage_count > 0 or partial_stage_count > 0

    final_report_markdown_path = None
    final_report_html_path = None
    if stages.get("report", {}).get("result"):
        final_report_markdown_path = stages["report"]["result"].get("final_report_v2", {}).get("markdown_path")
        final_report_html_path = stages["report"]["result"].get("final_report_v2", {}).get("html_path")
    mission_summary_markdown = format_mission_demo_summary(
        {
            "mission_id": mission_id,
            "output_root": str(mission_root),
            "success": success,
            "partial_success": partial_success,
            "stages": stages,
            "artifacts": list(dict.fromkeys([item for item in artifacts if item])),
            "final_report_markdown_path": final_report_markdown_path,
            "final_report_html_path": final_report_html_path,
            "truthfulness_note": "One-click demo orchestrator runs only available stages and never fabricates missing outputs.",
        }
    )
    ...
    return {
        ...
        "final_report_markdown_path": final_report_markdown_path,
        "final_report_html_path": final_report_html_path,
        "mission_summary_markdown": mission_summary_markdown,
        "truthfulness_note": "One-click demo orchestrator is a truthful stage coordinator; missing capabilities remain failed/skipped and are documented by scanner/ledger/report.",
        "skipped_stage_count": skipped_stage_count,
        "failed_stage_count": failed_stage_count,
    }


def format_mission_demo_summary(result):
    """Format one-click mission demo output for the UI or report preview."""
    if not isinstance(result, dict):
        return "## 一键任务演示\n\n结果格式无效。"

    lines = [
        "## 一键任务演示摘要",
        f"- mission_id：{result.get('mission_id', 'unknown')}",
        f"- output_root：{result.get('output_root', '')}",
        f"- success：{'是' if result.get('success') else '否'}",
        f"- partial_success：{'是' if result.get('partial_success') else '否'}",
        "",
    ]
    stages = result.get("stages", {}) or {}
    for stage_name in ["detection", "segmentation", "decision", "thermal", "report"]:
        stage = stages.get(stage_name, {})
        lines.extend(
            [
                f"### {stage_name}",
                f"- 状态：{stage.get('status', 'unknown')}",
                f"- 是否成功：{'是' if stage.get('success') else '否'}",
                f"- 说明：{stage.get('message', '')}",
                f"- 真实性说明：{stage.get('truthfulness_note', '')}",
                "",
            ]
        )

    artifacts = result.get("artifacts", []) or []
    lines.append("## 主要 artifacts")
    if artifacts:
    ...
    lines.extend(
        [
            "",
            f"- Final Report V2 Markdown：{result.get('final_report_markdown_path') or '未生成'}",
            f"- Final Report V2 HTML：{result.get('final_report_html_path') or '未生成'}",
            "",
            "## 真实性边界",
            "- 一键演示不会伪造未成功模块。",
            "- 未执行模块由 scanner / ledger / report 标记。",
            "- 路径规划是图像平面参考路径，不是真实 GPS。",
            "- 热红外真假由 thermal result 决定。",
            "- 报告是辅助决策，不替代人工判断。",
        ]
    )
    return "\n".join(lines)
