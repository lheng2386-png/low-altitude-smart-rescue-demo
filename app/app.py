...
def image_detection(
    ...
    transformer_summary_text = "Transformer RescueDet 未启用。YOLO 仍为当前主检测后端。"
...
AeroRescue-AI 是面向低空无人机应急救援场景的智能感知与辅助决策原型系统。系统围绕“无人机数据输入 → 灾情目标识别 → 环境风险理解 → 救援优先级排序 → 路径规划 → 灾情报告生成”构建完整演示链路，用于展示灾后城市区域的目标发现、风险研判和救援辅助决策能力。
...
2. 航测与场景预处理  
   对多张航测图像执行快速拼接预览，或在本机 Docker 与 OpenDroneMap 可用时运行真实 ODM 正射处理；对 360° 视频或多视角图像提取关键帧、特征点和匹配关系，为后续三维重建预处理提供基础数据。

3. 热红外与热点分析  
   系统支持普通图像的模拟热力分析，也预留真实 radiometric thermal 文件解析入口。普通 JPG 只生成模拟热点结果，只有成功解析温度矩阵的热红外文件才被标记为真实测温。

4. 目标检测与灾情感知  
   图像和视频输入进入目标检测模块。系统以 YOLOv11 灾害目标检测为主，同时提供 Transformer RescueDet 和双后端对比模式，用于识别平民、救援人员、动物等救援相关目标，并输出检测框、类别、置信度和目标位置。

5. 语义分割与环境风险融合  
   用户可以上传语义分割掩码，或在本地权重可用时启用自动分割模型。系统将水域、道路阻断、建筑损毁、可通行道路等环境类别转化为风险和路径代价，用于解释目标周边环境。

6. 灾损评估与场景适用性门控  
   系统统计建筑损毁、道路阻断、水域等灾损信息，并判断当前画面属于局部侦察还是广域评估。若缺少目标、分割掩码或模型权重，系统会进入 Target Only、Fallback 或 No Target 模式，避免在证据不足时给出过度结论。

7. TERP 救援优先级排序  
   TERP 模型综合目标类型、检测置信度、环境风险和可达性信息，对多个候选目标进行救援优先级排序。输出包括目标编号、综合分数、风险等级、排序原因和人工复核提示。

8. 普通 A* 与风险感知 A* 路径规划  
   系统在图像平面内生成参考救援路径，并对比普通 A* 路径与风险感知 A* 路径。风险感知路径会尽量避开水域、阻断道路和严重损毁区域。该路径是图像平面辅助参考，不是真实 GPS 导航路线。
...
| 功能页 | 主要用途 | 输出结果 |
| --- | --- | --- |
| 正射影像 / 航测拼接预览 | 检查航测照片质量、重叠关系和 ODM 环境，生成快速拼接或真实 ODM 输出 | 拼接预览图、处理状态、运行日志、结果 JSON |
| 模拟热红外 / 红外热点分析 | 对普通图像生成模拟热力图，或解析真实热红外温度矩阵 | 热力图、热点叠加图、真实性说明、温度/热点 JSON |
| 目标检测与综合决策 | 完成目标检测、语义分割融合、灾损评估、TERP 排序和路径规划 | 检测图、分割叠加图、路径图、排序表、救援报告 |
        ...
        with gr.Row():
            with gr.Column():
                ...
                transformer_model_key = gr.Dropdown(
                    ...
                    label="Transformer RescueDet 模型",
                    value="rescuedet_deformable_detr",
                )
                with gr.Accordion("目标检测后端能力说明", open=False):
                    gr.Markdown(summarize_detection_backend_capabilities())
                    gr.Markdown(
                        "### 运行时调度说明\n"
                        "- 当前可执行运行模式：YOLO Rescue Targets、Transformer RescueDet、YOLO + Transformer Compare。\n"
                        "- qazi / AIR / VTSaR 等仍是计划或参考后端，不进入当前 runtime 执行。\n"
                        "- 双后端对比中，TERP 和路径规划仍以 YOLO 主结果为准，Transformer 只作为人工复核线索。"
                    )
            ...
            with gr.Column():
                ...
                output_transformer_summary = gr.Textbox(
                    label="Transformer 辅助检测摘要",
                    lines=8,
                    placeholder="Transformer RescueDet 输出、失败原因或双后端一致性分析会显示在这里……",
                )
